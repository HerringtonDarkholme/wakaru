...
| 35 | `un-default-parameter` | partial | AST slice restores body-start loose defaults, common Babel `arguments[n]` ternaries, and logical-only boolean default forms into positional/default formal parameters. Existing formal parameters are now checked against the matched `arguments[n]` index before folding to avoid changing call semantics. Remaining gaps: broader scope/reference safety, exact TS generated-name parity, rest interaction, and full statement-container parity. |
...
| 35 | `un-default-parameter` | Broader scope/reference safety, exact TS generated-name parity, rest interaction, and full statement-container parity. Existing formal/`arguments[n]` index mismatches are now guarded. |
