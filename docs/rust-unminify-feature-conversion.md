...
| 26 | `un-esm` | partial | First AST slice is wired: runs Babel interop default/wildcard helpers, rebuilds existing imports plus top-level static `require` declarations/side-effect calls, dedupes named/bare imports, and annotates numeric `require(id)` as missing. Remaining gaps: namespace import hints, dynamic imports, hoist/fuzzy require, CJS export conversion, scope-safe conflict resolution for destructuring property access. |
| 27 | `un-enum` | partial | AST pass wired for the common TypeScript enum IIFE forms: declaration-paired IIFEs, mangled parameter names, string/numeric/heterogeneous members, invalid identifier keys, declaration merging with spread, terser `!function` calls, and esbuild arrow IIFE variable initializers. Remaining gaps: comment relocation parity and the upstream TODO SWC compressed assignment form. |
| 28 | `un-indirect-call` | partial | First semantic slice is wired for ESM default imports: `(0, s.useRef)()` becomes `useRef()`, existing named imports are reused, new named imports are added with conflict suffixes, shadowed default locals are ignored via Oxc symbol IDs, and default imports are removed only when all references were converted. CJS `require` destructuring insertion remains pending. |
...
| 30 | `smart-rename` | partial | First semantic slice is wired for object destructuring aliases in variable declarations and function parameters. It renames binding symbols and references to property names, resolves conflicts with suffixes, and prefixes reserved/invalid binding names. React ecosystem heuristics remain pending. |
| 31 | `smart-inline` | partial | First semantic slice is wired for adjacent single-`const` temp variable inlining, including chained temp assignments, only when Oxc resolved-reference counts show the temp binding has one use. Remaining gaps: object/array destructuring reconstruction, global identifier inlining, comments parity, and generated-name conflict handling for destructuring. |
| 32 | `un-optional-chaining` | partial | First AST slice is wired for canonical Babel/SWC ternary member guards: `(_a = a) === null || _a === void 0 ? void 0 : _a.b` becomes `a?.b`, including computed members and removal of unused temp declarations. Remaining gaps: logical `&&`/`||` truthy forms, nested decision-tree splitting, optional calls/call/apply/bind, delete, spread, parenthesized container parity, and semantic cleanup beyond simple temp declarations. |
| 33 | `un-nullish-coalescing` | partial | First AST slice is wired for canonical Babel/SWC ternary guards: `foo !== null && foo !== void 0 ? foo : fallback` becomes `foo ?? fallback`, including reversed operands, simple temp-assignment guards, and unused temp declaration removal. Remaining gaps: full decision-tree splitting, nested coalescing chains, logical-expression leaf forms like `&& value`, negated forms, optional-chaining interaction beyond simple expressions, and broader temp cleanup. |
| 34 | `un-conditionals` | partial | First AST slice is wired for expression-statement conditionals: simple ternaries become `if/else`, and simple `&&`/`||` guarded expressions become `if` statements with basic `!` negation folding. Remaining gaps: nested decision-tree rendering, return early-return rendering, switch reconstruction, logical trees beyond one level, and statement-container parity. |
...
| 37 | `un-parameters` | partial | Public composite registry entry is wired and currently runs the ported `un-default-parameter` slice. Add `un-parameter-rest` here when that pass is ported. |
