...
| 31 | `smart-inline` | partial | Semantic slice is wired for adjacent single-`const` temp variable inlining, chained temp assignments, browser-global alias inlining, array destructuring reconstruction from consecutive numeric member reads, and object destructuring reconstruction from repeated property reads with generated-name conflict handling. Remaining gaps: single property-access path renaming and comments parity. |
...
| ---: | --- | --- |
| 31 | `smart-inline` | Single property-access path renaming and comments parity. |
