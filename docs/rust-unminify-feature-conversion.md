...
| 26 | `un-esm` | partial | Runs Babel interop default/wildcard helpers, rebuilds existing imports plus top-level static `require` declarations/side-effect calls, dedupes named/bare imports, annotates numeric `require(id)` as missing, restores `Promise.resolve().then(() => require("x"))` dynamic imports, and converts the common top-level CJS export assignment forms. Remaining gaps: namespace import hints, hoist/fuzzy require, and scope-safe conflict resolution for destructuring property access. |
...
| ---: | --- | --- |
| 26 | `un-esm` | Namespace import hints, hoist/fuzzy require, and scope-safe conflict resolution for destructuring property access. |
