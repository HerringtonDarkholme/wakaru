...
| 26 | `un-esm` | partial | Runs Babel interop default/wildcard helpers, rebuilds existing imports plus top-level static `require` declarations/side-effect calls, dedupes named/bare imports, restores namespace imports from wildcard interop hints, converts `require("x").prop` destructuring with conflict-safe locals, annotates numeric `require(id)` as missing, restores `Promise.resolve().then(() => require("x"))` dynamic imports, and converts the common top-level CJS export assignment forms. Remaining gap: hoist/fuzzy require mode. |
...
| ---: | --- | --- |
| 26 | `un-esm` | Hoist/fuzzy require mode. |
