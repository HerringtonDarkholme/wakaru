...
| ---: | --- | --- |
| 32 | `un-optional-chaining` | Semantic cleanup beyond simple temp declarations. Nested conditional member chains, flattened logical guard chains, optional indirect calls, memoized call-argument recursion, apply-array argument recursion, and nested delete recovery are now covered. |
| 33 | `un-nullish-coalescing` | Full decision-tree splitting, optional-chaining interaction beyond simple expressions, and broader semantic cleanup. Nested coalescing chains, negated ternary guards, simple optional-chain logical leaves, and common containers are now covered. |
| 34 | `un-conditionals` | Switch reconstruction and broader statement-container parity. Nested expression-statement ternary/logical decision trees and direct `if` branch containers are now covered. |
| 35 | `un-default-parameter` | Broader scope/reference safety, exact TS generated-name parity, and full statement-container parity. Existing formal/`arguments[n]` index mismatches and mixed default/rest composite recovery are now covered. |
