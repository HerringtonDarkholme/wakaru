...
| 28 | `un-indirect-call` | done | Semantic transform for ESM default/namespace imports, top-level CJS `require` namespaces, and local namespace-like object bindings: indirect member calls like `(0, x.y)(...)` become direct local calls with named imports or object destructures reused/inserted, local conflicts get aliases, shadowed locals are ignored via Oxc symbol IDs, and default/namespace imports are removed only when all references were converted. |
...
| 32 | `un-optional-chaining` | done | AST slice is wired for canonical Babel/SWC ternary member guards, nested conditional member chains, computed members, optional direct calls, optional indirect `(0, fn)` calls, optional member calls, simple `.call`, `.apply`, and `.bind` optional calls, flattened logical `||` and `&&` truthy guard chains, recursive guard recovery in memoized call arguments and apply-array arguments, simple and nested `delete` recovery, pure-getter identifier guards, container positions from the TS fixture, and removal of generated temp declarations matched by the pass. |
| 33 | `un-nullish-coalescing` | done | AST slice is wired for canonical Babel/SWC ternary guards, reversed operands, simple temp-assignment guards, nested coalescing chains, negated `== null || == void 0` ternary guards, logical `&&` leaf forms, optional-chain logical leaves, default/destructuring/function containers, nested statement containers, and removal of generated temp declarations matched by the pass. |
| 34 | `un-conditionals` | done | AST slice is wired for expression-statement conditionals, nested expression-statement ternary/logical decision trees, simple `&&`/`||` guarded expressions with basic `!` negation folding, direct `if` branch expression-statement containers, nested ternary return rendering as early-return `if` statements, and switch/return-switch reconstruction for repeated equality comparisons against a common base. |
| 35 | `un-default-parameter` | done | AST slice restores body-start loose defaults, reversed loose guards, common Babel `arguments[n]` ternaries, logical-only boolean default forms, gap placeholder generation with TS fixture parity, function expressions, arrows, object methods/setters, and class methods into positional/default formal parameters. Existing formal parameters are checked against the matched `arguments[n]` index before folding to avoid changing call semantics, and mixed default/rest composite recovery is covered through `un-parameters`. |
| 36 | `un-parameter-rest` | done | Semantic slice converts parameterless non-arrow functions and methods that use implicit `arguments` into `...args`, including references from nested arrows, with scope checks for existing `args` bindings. It also recognizes Babel generated rest-copy loops with zero or existing-formal-parameter offsets. |
| 37 | `un-parameters` | done | Public composite registry entry is wired and runs the ported `un-default-parameter` and `un-parameter-rest` slices in TS order, including mixed default/rest parameter recovery. |
...
| 39 | `un-jsx` | done | Default-registry JSX recovery is ported for classic and automatic runtimes: string/component/member/namespaced tags, object/spread/computed props, React/Object spread helper flattening, children/key recovery, fragments, dynamic component hoisting with local name collision avoidance, constant string tag inlining, lowercase component binding renames, displayName-based renames, document.createElement exclusion, pure annotation cleanup through AST replacement/codegen, and CLI/runtime params for custom `pragma` / `pragmaFrag` names. |
...
| ---: | --- | --- |
| - | - | No active partial-pass items. Add a new row only after an audit finds a concrete uncovered pass slice. |
