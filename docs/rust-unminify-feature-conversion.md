...
| 26 | `un-esm` | partial | Runs Babel interop default/wildcard helpers, rebuilds existing imports plus top-level static `require` declarations/side-effect calls, dedupes named/bare imports, restores namespace imports from wildcard interop hints, annotates numeric `require(id)` as missing, restores `Promise.resolve().then(() => require("x"))` dynamic imports, and converts the common top-level CJS export assignment forms. Remaining gaps: hoist/fuzzy require and scope-safe conflict resolution for destructuring property access. |
...
| 31 | `smart-inline` | partial | Semantic slice is wired for adjacent single-`const` temp variable inlining, chained temp assignments, and array destructuring reconstruction from consecutive numeric member reads, only when Oxc resolved-reference counts show safe single-use temps. Remaining gaps: object destructuring reconstruction, global identifier inlining, comments parity, and generated-name conflict handling for destructuring. |
| 32 | `un-optional-chaining` | partial | AST slice is wired for canonical Babel/SWC ternary member guards, computed members, optional direct calls, optional member calls, simple `.call` optional calls, and removal of unused temp declarations. Remaining gaps: logical `&&`/`||` truthy forms, nested decision-tree splitting, `.apply`/`.bind`, delete, spread, parenthesized container parity, and semantic cleanup beyond simple temp declarations. |
...
| ---: | --- | --- |
| 26 | `un-esm` | Hoist/fuzzy require and scope-safe conflict resolution for destructuring property access. |
