...
const MODULE_NAME: &str = "@babel/runtime/helpers/arrayLikeToArray";
const MODULE_ESM_NAME: &str = "@babel/runtime/helpers/esm/arrayLikeToArray";
...
pub fn transform_ast(source: &mut ParsedSourceFile) -> Result<()> {
    ...
    let mut restorer = ArrayLikeToArrayRestorer {
...
struct ArrayLikeToArrayRestorer<'a> {
    ast: AstBuilder<'a>,
    helper_locals: Vec<String>,
    processed_counts: HashMap<String, usize>,
}

impl<'a> VisitMut<'a> for ArrayLikeToArrayRestorer<'a> {
    fn visit_expression(&mut self, expression: &mut Expression<'a>) {
        walk_mut::walk_expression(self, expression);

        let Some(helper_local) = self.match_array_like_to_array_call(expression) else {
            return;
        };
        ...
        let call = call.unbox();
        let Some(Argument::ArrayExpression(array)) = call.arguments.into_iter().next() else {
            return;
        };

        *expression = self.array_with_undefined_holes(span, array);
        *self.processed_counts.entry(helper_local).or_default() += 1;
    }
}

impl<'a> ArrayLikeToArrayRestorer<'a> {
    fn match_array_like_to_array_call(&self, expression: &Expression<'a>) -> Option<String> {
    ...
    fn array_with_undefined_holes(
        &self,
        span: oxc_span::Span,
        mut array: oxc_allocator::Box<'a, ArrayExpression<'a>>,
    ) -> Expression<'a> {
        let old_elements = array.elements.take_in(self.ast);
        let mut elements = self.ast.vec_with_capacity(old_elements.len());
            ...
            match element {
                ArrayExpressionElement::Elision(elision) => {
                    elements.push(ArrayExpressionElement::Identifier(
                        self.ast
                            .alloc_identifier_reference(elision.span, "undefined"),
                    ));
                }
...
mod tests {
    ...
    #[test]
    fn restores_cjs_array_like_to_array_calls() {
...
",
    ...
    #[test]
    fn restores_esm_array_like_to_array_calls() {
        define_ast_inline_test(transform_ast)(
            r#"
...
",
    ...
    #[test]
    fn accepts_numeric_length_argument() {
        define_ast_inline_test(transform_ast)(
            r#"
var _arrayLikeToArray = require("@babel/runtime/helpers/arrayLikeToArray");
...
",
    ...
    #[test]
    fn leaves_invalid_length_argument() {
        define_ast_inline_test(transform_ast)(
            r#"
var _arrayLikeToArray = require("@babel/runtime/helpers/arrayLikeToArray");

_arrayLikeToArray([1,,3], n);
"#,
            r#"
var _arrayLikeToArray = require("@babel/runtime/helpers/arrayLikeToArray");
_arrayLikeToArray([
